...
# Model to represents a single player round
class Pinfall
  attr_accessor :quantities

  AMOUNT_ERROR_MESSAGE = 'The amount must be F or be between 0 and 10'.freeze
  SIZE_ERROR_MESSAGE   = 'This round has exactly two moves'.freeze
  ...
  def related_total_falled_pins
    return [quantities_integer[0..1].sum, 10].min if three_times?
